...
public class JwtAuthenticationProcessingFilter extends OncePerRequestFilter {
  ...
  /**
   * 1. 리프레시 토큰이 오는 경우 -> 유효하면 AccessToken 재발급후, 필터 진행 X, 바로 튕기기 2. 리프레시 토큰은 없고 AccessToken만 있는 경우
   * -> 유저정보 저장후 필터 계속 진행
   */
  @Override
  ...
  private void checkAccessTokenAndAuthentication(HttpServletRequest request,
    ...
    jwtService.extractAccessToken(request).filter(jwtService::isTokenValid).ifPresent(
        ...
        accessToken -> jwtService.extractUsermail(accessToken).ifPresent(

            usermail -> memberRepository.findByEmail(usermail).ifPresent(
