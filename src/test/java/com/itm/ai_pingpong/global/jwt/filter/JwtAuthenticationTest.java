package com.itm.ai_pingpong.global.jwt.filter;
...
class JwtAuthenticationTest {
  ...
  private static String USERMAIL = "usermail@test.com";
  ...
  private Map getUsermailPasswordMap(String usermail, String password) {
    Map<String, String> map = new HashMap<>();
    map.put(KEY_USERMAIL, usermail);
    map.put(KEY_PASSWORD, password);
    return map;
  }

  private Map getAccessAndRefreshToken() throws Exception {
    Map<String, String> map = getUsermailPasswordMap(USERMAIL, PASSWORD);
   ...
   */
  @Test
  void AccessToken만_보내서_인증() throws Exception {
    //given
   ...
   */
  @Test
  void 안유효한AccessToken만_보내서_인증x_상태코드는_403() throws Exception {
    //given
    Map accessAndRefreshToken = getAccessAndRefreshToken();
    String accessToken = (String) accessAndRefreshToken.get(accessHeader);
  ...
  /**
   * AccessToken : 존재하지 않음 RefreshToken : 유효
   */
  @Test
  void 유효한RefreshToken만_보내서_AccessToken_재발급_200() throws Exception {
    //given
    Map accessAndRefreshToken = getAccessAndRefreshToken();
   ...
   */
  @Test
  void 유효한RefreshToken이랑_유효한AccessToken_같이보낼경우_AccessToken_재발급_200() throws Exception {
    //given
    Map accessAndRefreshToken = getAccessAndRefreshToken();
    String accessToken = (String) accessAndRefreshToken.get(accessHeader);
    String refreshToken = (String) accessAndRefreshToken.get(refreshHeader);
  ...
  /**
   * AccessToken : 유효하지 않음 RefreshToken : 유효
   */
  @Test
  void 유효한RefreshToken이랑_안유효한AccessToken_같이보냈을때_AccessToken_재발급_200() throws Exception {
    //given
    Map accessAndRefreshToken = getAccessAndRefreshToken();
    String accessToken = (String) accessAndRefreshToken.get(accessHeader);
    String refreshToken = (String) accessAndRefreshToken.get(refreshHeader);
  ...
  /**
   * AccessToken : 유효 RefreshToken : 유효하지 않음
   */
  @Test
  void 안유효한RefreshToken이랑_유효한AccessToken_같이보냈을때_상태코드200_혹은404_RefreshToken은_AccessToken모두_재발급되지않음()
      throws Exception {
    //given
    Map accessAndRefreshToken = getAccessAndRefreshToken();
    String accessToken = (String) accessAndRefreshToken.get(accessHeader);
   ...
   */
  @Test
  void 안유효한RefreshToken이랑_안유효한AccessToken_같이보냈을때_403() throws Exception {
    //given
    Map accessAndRefreshToken = getAccessAndRefreshToken();
    String accessToken = (String) accessAndRefreshToken.get(accessHeader);
    String refreshToken = (String) accessAndRefreshToken.get(refreshHeader);
  ...
  @Test
  void 로그인_주소로_보내면_필터작동_X() throws Exception {
    //given
    Map accessAndRefreshToken = getAccessAndRefreshToken();
    String accessToken = (String) accessAndRefreshToken.get(accessHeader);
    String refreshToken = (String) accessAndRefreshToken.get(refreshHeader);
    ...
    MvcResult result = mockMvc.perform(post(LOGIN_URL)  //get인 경우 config에서 permitAll을 했기에 notFound
            .header(refreshHeader, BEARER + refreshToken)
            .header(accessHeader, BEARER + accessToken))
        .andExpect(status().isBadRequest()) // post요청에 body가 없기때문에
        .andReturn();

    String contentAsString = result.getResponse().getContentAsString();
    assertThat(contentAsString).isEqualTo("fail");
